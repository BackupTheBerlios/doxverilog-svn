...
bool VerilogDocGen::bClassHeader=false;
...
QCString argType,temp,ltype,largs,lequ;
 ...
 if(parseCode) return;
  ...
  if(bClassHeader && currentVerilog)
	  {
	  currentVerilog->args+=mod.simplifyWhiteSpace();
	  }
  int u=mod.find('(');
  int v=mod.find('#');
...
void VerilogDocGen::parseClassHeader()
{
     if(VerilogDocGen::parseCode) return;
							
						     QCString jj=getVerilogString(); 
							 VhdlDocGen::deleteAllChars(jj,';');
							 jj=jj.simplifyWhiteSpace();
							 if(!jj.isEmpty())
							   jj.prepend("(");
							 
							 if(jj.data() ||currentVerilog->args.contains('#'))
								 {
							       currentVerilog->args+=" "+jj;
                                   Argument *arg=new Argument();
							       arg->name=currentVerilog->args;
							       ArgumentList *aList=new ArgumentList;
							       aList->append(arg);
							       currentVerilog->tArgLists=new QList<ArgumentList>;
							       currentVerilog->tArgLists->append(aList);
								 }
							bClassHeader=false;
							portType.resize(0);
						     vbufreset();
